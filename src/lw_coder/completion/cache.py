...
class PlanCompletionCache:
    ...
    def get_active_plans(self, tasks_dir: Path | None = None) -> list[str]:
        ...
        Scans .lw_coder/tasks/*.md files and returns plan IDs (file stems)
        for plans where status != "done" in YAML front matter.

        Args:
            tasks_dir: Path to .lw_coder/tasks directory. If None, attempts to
        ...
        if tasks_dir is None:
            try:
                ...
                logger.debug("Could not find .lw_coder/tasks directory")
                return []
...
def get_active_plans(tasks_dir: Path | None = None) -> list[str]:
    ...
    Args:
        tasks_dir: Path to .lw_coder/tasks directory.
