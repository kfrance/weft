...
def copy_plan_files(
    ...
    Args:
        source_dir: Source directory (worktree's .lw_coder/tasks/).
        dest_dir: Destination directory (main repo's .lw_coder/tasks/).
        existing_files: Set of filenames that existed before execution.
    ...
    for source_file in new_files:
        try:
            ...
            if target_filename == source_file.name:
                logger.info(
                    "Saved plan to .lw_coder/tasks/%s", target_filename
                )
            else:
                logger.info(
                    "Saved plan to .lw_coder/tasks/%s (original name %s already existed)",
                    target_filename,
                    source_file.name,
                )
