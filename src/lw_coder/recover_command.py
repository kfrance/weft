    ...
    try:
        ...
        if total_count == 0:
            if show_abandoned:
                ...
                print("\nPlans are moved to abandoned when you run 'lw_coder abandon'.")
            elif show_all:
        ...
        # Display help text
        if show_abandoned:
            print(f"\nUse 'lw_coder recover-plan --abandoned <plan_id>' to restore an abandoned plan.")
        elif show_all:
            print(f"\nUse 'lw_coder recover-plan <plan_id>' for active backups.")
            print(f"Use 'lw_coder recover-plan --abandoned <plan_id>' for abandoned plans.")
        else:
    ...
    try:
        ...
        if from_abandoned:
            ...
            if not backup_exists_in_namespace(repo_root, plan_id, namespace):
                ...
                print("Tip: Use 'lw_coder recover-plan --abandoned' to list available abandoned plans.")
                return 1
        else:
            ...
            if not backup_exists_in_namespace(repo_root, plan_id, namespace):
                ...
                if backup_exists_in_namespace(repo_root, plan_id, "plan-abandoned"):
                    ...
                    print(f"  lw_coder recover-plan --abandoned {plan_id}")
                    return 1
        ...
        logger.error("%s", exc)
        if from_abandoned:
            print(f"\nTip: Use 'lw_coder recover-plan --abandoned' to list available abandoned plans.")
        else:
