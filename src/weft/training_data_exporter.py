...
Training data is stored in .weft/training_data/<plan_id>/ and is
...
def copy_plan_file(plan_id: str, repo_root: Path, staging_dir: Path) -> None:
    ...
    """
    source = repo_root / ".weft" / "tasks" / f"{plan_id}.md"
...
def copy_code_trace(
    ...
    """
    source = repo_root / ".weft" / "sessions" / plan_id / "code" / "trace.md"
...
def copy_ai_patch(plan_id: str, repo_root: Path, staging_dir: Path) -> None:
    ...
    """
    source = repo_root / ".weft" / "sessions" / plan_id / "code" / "ai_changes.patch"
...
def copy_test_results(plan_id: str, repo_root: Path, staging_dir: Path) -> None:
    ...
    """
    eval_dir = repo_root / ".weft" / "sessions" / plan_id / "eval"
...
def copy_human_feedback(plan_id: str, repo_root: Path, staging_dir: Path) -> None:
    ...
    """
    source = (
        repo_root / ".weft" / "sessions" / plan_id / "eval" / "human_feedback.md"
    )
...
def copy_prompts(plan_id: str, repo_root: Path, staging_dir: Path) -> Optional[str]:
    ...
    """
    source = repo_root / ".weft" / "sessions" / plan_id / "code" / "prompts"
...
def copy_and_update_metadata(
    ...
    """
    source = repo_root / ".weft" / "sessions" / plan_id / "code" / "metadata.json"
...
def create_training_data(plan_id: str, repo_root: Path, eval_fingerprint: str) -> Path:
    ...
    """
    training_data_dir = repo_root / ".weft" / "training_data" / plan_id
