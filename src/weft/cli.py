...
def create_parser() -> argparse.ArgumentParser:
    """Create the argument parser for weft CLI.
    ...
    init_parser = subparsers.add_parser(
        "init",
        help="Initialize weft in current git repository",
    )
    init_parser.add_argument(
        "--force",
        dest="force",
        action="store_true",
        help="Reinitialize even if .weft already exists",
    )
    init_parser.add_argument(
        "--yes",
        dest="yes",
    ...
    completion_subparsers.add_parser(
        "install",
        help="Install bash completion for weft",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    # Auto-migrate .lw_coder/ to .weft/ if needed (for repos created before rebrand)
    # This runs early for all commands except when we're not in a repo context
    if args.command not in ("init", None):
        try:
            from .repo_utils import find_repo_root, migrate_repo_dir_if_needed, RepoUtilsError
            repo_root = find_repo_root()
            migrate_repo_dir_if_needed(repo_root)
        except RepoUtilsError as e:
            # Migration failure - log warning but allow command to continue
            # The command itself may fail later, but at least user sees why
            logger.warning("Failed to migrate .lw_coder to .weft: %s", e)
        except Exception:
            # Not in a git repo or other expected error - skip silently
            pass
