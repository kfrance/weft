...
def get_worktree_path(repo_root: Path, plan_id: str) -> Path:
    ...
    Returns:
        Path to the worktree directory (.weft/worktrees/<plan_id>).
    ...
    # Ensure resolved path is within repo_root/.weft/worktrees
    # Use resolve() to handle any symbolic links or relative components
    try:
        expected_parent = (repo_root / ".weft" / "worktrees").resolve()
...
def validate_worktree_exists(repo_root: Path, plan_id: str) -> Path:
    ...
    if not worktree_path.exists():
        raise WorktreeError(
            ...
            f"Run 'weft code' first to create the worktree."
        )
