...
project's .weft directory, replacing the dynamic DSPy-based prompt generation.

Directory structure:
- New location: .weft/prompts/active/<tool>/<model>/
...
def _migrate_prompts_if_needed(repo_root: Path) -> bool:
    ...
    """
    old_location = repo_root / ".weft" / "optimized_prompts"
    new_location = repo_root / ".weft" / "prompts" / "active"
...
def _get_prompts_base(repo_root: Path, tool: str, model: str) -> Path:
    ...
    new_location = repo_root / ".weft" / "prompts" / "active" / tool / model
    if new_location.exists():
        return new_location

    # Fall back to old location
    old_location = repo_root / ".weft" / "optimized_prompts" / tool / model
    ...
    for key, file_path in prompt_files.items():
        if not file_path.exists():
            raise PromptLoadingError(
                ...
                f"Expected at .weft/prompts/active/{tool}/{model}/{file_path.name}"
            )
...
def load_current_prompts_for_training(
    ...
    if not prompts_base.exists():
        raise PromptLoadingError(
            ...
            f"Run 'weft init' to install prompts."
        )
