...
These tests verify CLI validation behavior by invoking lw_coder as a subprocess
...
def run_lw_coder(*args: str, cwd: Path | None = None) -> subprocess.CompletedProcess[str]:
    """Run lw_coder CLI as a subprocess and return the result.

    Args:
        *args: Command-line arguments to pass to lw_coder.
...
class TestCliInitCommand:
    ...
    def test_init_fails_outside_git_repo(self) -> None:
        """Test that lw_coder init outside git repo returns exit code 1."""
        with tempfile.TemporaryDirectory() as tmpdir:
            # Ensure no .git directory exists
            result = run_lw_coder("init", cwd=Path(tmpdir))
    ...
    def test_init_force_yes_completes(self, tmp_path: Path) -> None:
        """Test that lw_coder init --force --yes succeeds non-interactively."""
        ...
        result = run_lw_coder("init", "--force", "--yes", cwd=tmp_path)

        assert result.returncode == 0, f"init failed: {result.stderr}"
        # Verify .lw_coder directory was created
        lw_coder_dir = tmp_path / ".lw_coder"
        assert lw_coder_dir.exists(), ".lw_coder directory should be created"
