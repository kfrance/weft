...
def test_load_prompts_success(tmp_path: Path) -> None:
    ...
    prompts_dir = tmp_path / ".weft" / "optimized_prompts" / "claude-code" / "sonnet"
    ...
    result = load_prompts(repo_root=tmp_path, tool="claude-code", model="sonnet")
...
def test_load_prompts_all_models(tmp_path: Path) -> None:
    ...
    for model in models:
        prompts_dir = tmp_path / ".weft" / "optimized_prompts" / "claude-code" / model
        prompts_dir.mkdir(parents=True)
    ...
    for model in models:
        result = load_prompts(repo_root=tmp_path, tool="claude-code", model=model)
...
def test_load_prompts_invalid_model(tmp_path: Path) -> None:
    ...
    with pytest.raises(PromptLoadingError) as exc_info:
        load_prompts(repo_root=tmp_path, tool="claude-code", model="invalid-model")
...
def test_load_prompts_missing_file(tmp_path: Path, missing_file: str, other_files: list[str], model: str) -> None:
    ...
    with pytest.raises(PromptLoadingError) as exc_info:
        load_prompts(repo_root=tmp_path, tool="claude-code", model=model)
...
def test_load_prompts_file_read_error(tmp_path: Path) -> None:
    ...
    with patch("pathlib.Path.read_text", side_effect=OSError("Permission denied")):
        with pytest.raises(PromptLoadingError) as exc_info:
            load_prompts(repo_root=tmp_path, tool="claude-code", model="sonnet")
...
def test_load_finalize_prompt_from_repo_specific_location(tmp_path: Path) -> None:
    ...
    prompt_dir = tmp_path / ".weft" / "prompts" / "active" / "claude-code"
...
def test_load_finalize_prompt_auto_copy_from_bundled(tmp_path: Path) -> None:
    ...
    copied_file = tmp_path / ".weft" / "prompts" / "active" / "claude-code" / "finalize.md"
...
def test_load_finalize_prompt_auto_copy_is_idempotent(tmp_path: Path) -> None:
    ...
    with patch("weft.paths.get_weft_src_dir") as mock_src_dir:
        ...
        result1 = load_finalize_prompt(tmp_path, "claude-code")
        # Second call should load from repo-specific location
        result2 = load_finalize_prompt(tmp_path, "claude-code")
...
def test_load_finalize_prompt_error_when_bundled_missing(tmp_path: Path) -> None:
    ...
    with patch("weft.paths.get_weft_src_dir") as mock_src_dir:
        ...
        with pytest.raises(PromptLoadingError) as exc_info:
            load_finalize_prompt(tmp_path, "claude-code")
...
def test_load_finalize_prompt_creates_parent_directories(tmp_path: Path) -> None:
    ...
    assert (tmp_path / ".weft" / "prompts" / "active" / "claude-code" / "finalize.md").exists()
